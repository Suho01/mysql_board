    ...
    if (req.method === 'POST') {
        try {
            ...
            if (!parentid || !userid || !username || !content) {
                ...
                const [datas] = await db.query<RowDataPacket[]>('select * from suho.comment where parentid = ?', [parentid]);
...
export const GET = async (
    ...
    if (req.method === 'GET') {
        try {
            ...
            const [results] = await db.query<RowDataPacket[]>('select * from suho.comment where parentid = ?', [parentid]);
